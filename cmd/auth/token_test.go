...
func TestTokenDockerFormatEmitsGetResponse(t *testing.T) {
	...
	cmd.SetContext(ctx)
	cmd.SetIn(strings.NewReader("123456789.containers.us-west-2.cloud.databricks.com\n"))
	cmd.SetOut(&stdout)
...
func TestWriteDockerTokenOutputUsesConfiguredProfiler(t *testing.T) {
	...
	require.Equal(t, "workspace", gotProfile)
	require.JSONEq(t, `{"Username":"oauthtoken","Secret":"access-token"}`, stdout.String())
}
...
func TestTokenDockerFormatRejectsUnsupportedProfile(t *testing.T) {
	...
	for _, registryHost := range []string{
		...
		"987654321.containers.us-west-2.cloud.databricks.com",
		"111222333.containers.us-west-2.cloud.databricks.com",
	} {
