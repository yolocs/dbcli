...
func TestConfigureDockerCommandWritesDockerConfigAndShim(t *testing.T) {
	...
	cmd := newConfigureDockerTestCommand(ctx, "configure-docker", "DEFAULT", "--region", "us-west-2")
	require.NoError(t, cmd.Execute())

	registryHost := "123456789.containers.us-west-2.cloud.databricks.com"
...
func TestConfigureDockerCommandPersistsResolvedWorkspaceID(t *testing.T) {
	...
	helpers := readCredentialHelpers(t, filepath.Join(dockerDir, "config.json"))
	require.Equal(t, dockercredentials.HelperName, helpers["999999.containers.us-west-2.cloud.databricks.com"])
}
